...
#Region "添加"
        ...
        Public Function MrpForecastOrderEntry_Add(ByVal objInfo As MrpForecastOrderEntryInfo) As Boolean
            Try
                ...
                db.AddInParameter(dbComm, "@MI_Qty", DbType.Decimal, objInfo.MI_Qty)
...
#Region "更新"
        ...
        Public Function MrpForecastOrderEntry_Update(ByVal objinfo As MrpForecastOrderEntryInfo) As Boolean
            Try
                ...
                db.AddInParameter(dbComm, "@DateCross", DbType.String, objinfo.DateCross)
                If objinfo.PlanQty = Nothing Then
                    db.AddInParameter(dbComm, "@PlanQty", DbType.Decimal, DBNull.Value)
                Else
        ...
        Public Function MrpForecastOrderEntry_UpdateMrpOperation(ByVal ForecastID As String, ByVal NeedDate As Date) As Boolean
            Try
                Dim db As New Microsoft.Practices.EnterpriseLibrary.Data.Sql.SqlDatabase(ConnStr)
                Dim dbComm As DbCommand = db.GetStoredProcCommand("MrpForecastOrderEntry_UpdateMrpOperation")
                db.AddInParameter(dbComm, "@ForecastID", DbType.String, ForecastID)
                db.AddInParameter(dbComm, "@MI_NeedDate", DbType.Date, NeedDate)
                db.ExecuteNonQuery(dbComm)
                Return True
            Catch ex As Exception
                MsgBox(ex.Message)
        ...
        Public Function MrpForecastOrderEntry_Check(ByVal objinfo As MrpForecastOrderEntryInfo) As Boolean
            Try
                Dim db As New Microsoft.Practices.EnterpriseLibrary.Data.Sql.SqlDatabase(ConnStr)
                Dim dbComm As DbCommand = db.GetStoredProcCommand("MrpForecastOrderEntry_Check")
                ...
                db.ExecuteNonQuery(dbComm)
                MrpForecastOrderEntry_Check = True
            Catch ex As Exception
                MsgBox(ex.Message)
                MrpForecastOrderEntry_Check = False
            End Try
        End Function
...
#Region "填充"
        ...
        Friend Function FillMrpForecastOrderEntry(ByVal reader As IDataReader) As MrpForecastOrderEntryInfo
            ...
            End If
            If IsDBNull(reader("IsMrpOperation")) Then
                objInfo.IsMrpOperation = False
            Else
                objInfo.IsMrpOperation = reader("IsMrpOperation")
            End If

            If IsDBNull(reader("PlanQty")) = True Then
                objInfo.PlanQty = 0
            Else
                objInfo.PlanQty = reader("PlanQty")
            End If

            If IsDBNull(reader("DeliveryQty")) = True Then
                objInfo.DeliveryQty = 0
            Else
                objInfo.DeliveryQty = reader("DeliveryQty")
            End If
            If IsDBNull(reader("IsCheck")) Then
                objInfo.IsCheck = False
            Else
            ...
            End If
            If IsDBNull(reader("CheckUserID")) = True Then
