    ...
    Public Class MrpInfoController
        ...
        Friend Function FillMrpInfo(ByVal reader As IDataReader) As MRPInfoInfo
            Try
                ...
                objInfo.PR_ID = reader("PR_ID").ToString
                Return objInfo
        ...
        Public Function MrpInfo_GetList(ByVal MRP_ID As String, ByVal BeginDate As DateTime, ByVal CheckType As String, ByVal MrpType As String, ByVal MI_CreateUserID As String, ByVal EndDate As DateTime, ByVal MI_CalcType As Integer, ByVal MI_MRPType As String, ByVal MI_CheckBit As String, ByVal DisplayNum As Decimal) As List(Of MrpInfoInfo)
            Try
                ...
                If EndDate <> Nothing Then
                    db.AddInParameter(dbComm, "@BeginDate", DbType.Date, EndDate)
                End If
                If MI_CalcType <> Nothing Then
                    db.AddInParameter(dbComm, "@MI_CalcType", DbType.Int16, MI_CalcType)
                End If
                If MI_MRPType <> Nothing Then
                    db.AddInParameter(dbComm, "@MI_MRPType ", DbType.Boolean, MI_MRPType)
                End If
                If MI_CheckBit <> Nothing Then
                    db.AddInParameter(dbComm, "@MI_CheckBit ", DbType.Boolean, MI_CheckBit)
                End If
                If CheckType <> Nothing Then
                    db.AddInParameter(dbComm, "@CheckType", DbType.String, CheckType)
                End If
                If MrpType <> Nothing Then
                    db.AddInParameter(dbComm, "@MrpType", DbType.String, MrpType)
                End If
                If MI_CreateUserID <> Nothing Then
        ...
        Public Function MrpInfo_PreDelete(ByVal MRP_ID As String) As Boolean
            Dim list As New List(Of String)
            list.Add("select MrpPurchaseID from MrpPurchaseRecord where MRP_ID='" + MRP_ID + "'")
            list.Add("的請購單中存在該運算單號(" + MRP_ID + ")，無法刪除")

            list.Add("select Ware_ID from MrpWareHouseInfo  where MRP_ID='" + MRP_ID + "'")
            list.Add("的庫存單中存在該運算單號(" + MRP_ID + ")，無法刪除")
        ...
        Public Function MrpInfo_UpdateCheck(ByVal objInfo As MRPInfoInfo) As Boolean
            Try
                ...
                db.AddInParameter(dbComm, "@PR_ID", DbType.String, objInfo.PR_ID)
                db.ExecuteNonQuery(dbComm)
        ...
        '    Dim sqlStr As String = "select M_Code,M_Name,M_Gauge,M_Unit,(select Source from MrpSource,MrpMaterialCode where MC_Source=SourceID and M.M_code=M_Code) as Source from MaterialCode AS M"
        '    Dim db As New Microsoft.Practices.EnterpriseLibrary.Data.Sql.SqlDatabase(ConnStr)
        '    Dim dbcomm As DbCommand = db.GetSqlStringCommand(sqlStr)
        '    Return db.ExecuteDataSet(dbcomm).Tables(0)
        'End Function

        Public Function GetMrpWareHouseID(ByVal tempMrpID As String) As String
            Dim db As New Microsoft.Practices.EnterpriseLibrary.Data.Sql.SqlDatabase(ConnStr)
...
#Region "MRP運算時取得請購建議表"
        Public Sub GetPurchaseMaterial(ByVal needBeginDate As Date, ByVal needEndDate As Date, ByVal CheckType As String, ByVal Cancellation As String, ByVal ForecastID As String, ByVal WareID As String)
            ...
            Try
                ...
                Using reader As IDataReader = db.ExecuteReader(dbcomm)
                    While reader.Read
                        GetPurchaseQty(reader("M_Code").ToString, CDec(reader("NeedQty")), WareID, CDate(reader("NeedDate")))
                    End While
                End Using
            Catch ex As Exception
                MsgBox(ex.Message, MsgBoxStyle.Information, "Proc:Mrp_GetIndReq")
        ...
        Private Sub GetPurchaseQty(ByVal M_Code As String, ByVal NeedQty As Decimal, ByVal WareID As String, ByVal NeedDate As Date)
            Try
                Dim row As DataRow
                Dim db As New Microsoft.Practices.EnterpriseLibrary.Data.Sql.SqlDatabase(ConnStr)
...
#Region "Mrp運算稽核"
        Public Function Mrp_GetComputingAuditing(ByVal needBeginDate As Date, ByVal needEndDate As Date, ByVal CheckType As String, ByVal Cancellation As String, ByVal ForecastID As String) As DataSet
            Try
                Dim db As New Microsoft.Practices.EnterpriseLibrary.Data.Sql.SqlDatabase(ConnStr)
                Dim dbcomm As DbCommand = db.GetStoredProcCommand("Mrp_GetComputingAuditing_GetList")
                If needBeginDate = Nothing Then
                    db.AddInParameter(dbcomm, "@NeedBeginDate", DbType.Date, DBNull.Value)
                Else
                    db.AddInParameter(dbcomm, "@NeedBeginDate", DbType.Date, needBeginDate)
            ...
            Catch ex As Exception
                MsgBox(ex.Message, MsgBoxStyle.Information, "中間層：Mrp_GetComputingAuditing_GetList")
                Return Nothing
            End Try
        End Function

        Public Function Mrp_WareIDAuditing(ByVal wareID As String) As Boolean
            Dim db As New Microsoft.Practices.EnterpriseLibrary.Data.Sql.SqlDatabase(ConnStr)
            Dim dbcomm As DbCommand = db.GetSqlStringCommand("select Ware_ID from MrpWareHouseInfo where CheckBit=0 and Ware_ID='" + wareID + "'")
            Using reader As SqlDataReader = db.ExecuteReader(dbcomm)
                If reader.Read Then
                    Return False
                Else
