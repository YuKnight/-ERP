...
#Region "判空"
    ...
    Function DataCheckEmpty() As Integer
        'If MWHIcon.MrpWareHouseInfo_MRPIDExists(gueMRPID.EditValue.ToString) = True And EditItem = "Add" Then
        '    MsgBox("刻計劃運算編碼已存在，請重新選擇計劃運算編碼！", MsgBoxStyle.Information, "提示")
        '    DataCheckEmpty = 0
        '    Exit Function
        'End If
        If Me.gueMRPID.EditValue = String.Empty Then
            MsgBox("計畫運算編碼不能為空,請輸入！", MsgBoxStyle.Information, "提示")
            gueMRPID.Focus()
            DataCheckEmpty = 0
            Exit Function
        End If
            ...
            For j = i + 1 To ds.Tables("MrpWareHouseInfoEntry").Rows.Count - 1
                With ds.Tables("MrpWareHouseInfoEntry")
                    If .Rows(i)("M_Code").ToString = .Rows(j)("M_Code").ToString Then
                        MsgBox("不能存在重復產品:" + .Rows(i)("M_Code").ToString, MsgBoxStyle.Information, "提示")
                        DataCheckEmpty = 0
                        Exit Function
                    End If
                End With
...
#Region "載入數據"
    ...
    Sub LoadData(ByVal StrWare_ID As String)
        ...
        MWHI_list = MWHIcon.MrpWareHouseInfo_GetList(StrWare_ID, Nothing, Nothing, Nothing)
...
#Region "載入事件"
    ...
    Private Sub frmMrpWareHouseInfoAdd_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        gueMRPID.Properties.DataSource = mc.MrpInfo_GetList(Nothing, Nothing, Nothing, Nothing, Nothing)
        Select Case EditItem
            Case "Add"                
                ...
                If (DataList Is Nothing = False) Then
                    ...
                    Exit Sub
                End If
                Grid3.DataSource = MMICcon.MrpMaterialCode_GetList(Nothing, Nothing, Nothing, Nothing, Nothing)
            Case "Edit"
...
#Region "新增/刪除子表"
    ...
    Private Sub cmsAddSub_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmsAddSub.Click
        ...
        row("M_Unit") = Nothing
        row("Source") = Nothing
        row("MP_InventoryQty") = Nothing
        row("MP_InTransitQty") = Nothing
        row("MP_Inspection") = Nothing
        row("MP_NoCollar") = Nothing
        row("MP_RetreatQty") = Nothing
