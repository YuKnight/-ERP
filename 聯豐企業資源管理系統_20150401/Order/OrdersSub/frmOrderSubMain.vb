...
Public Class frmOrderSubMain

    Private Sub frmOrderSubMain_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        PopRefresh_Click(Nothing, Nothing)           '@ 2012/9/20 修改 調用刷新過程
       
        LoadPowerUser()


        popOrdersSubNeed.Visible = False
    End Sub
    ...
    Private Sub popOrderSubEdit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles popOrderSubEdit.Click
    ...
    Private Sub popOrderSubDel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles popOrderSubDel.Click
    ...
    ''' frmOrderSubMain_Load()
    Private Sub PopRefresh_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles PopRefresh.Click
        Dim osc As New OrdersSubController
     
        Grid1.DataSource = osc.OrdersSub_GetList1(Nothing, Nothing, Nothing, Nothing, "大貨批次", Nothing, Nothing)             '@ 2012/9/20 修改 限制只顯示500條記錄
    End Sub

    Private Sub PopOrderSubPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles PopOrderSubPrint.Click
        '測試報表用
        On Error Resume Next
        ...
        'ltc1.CollToDataSet(ds, "OrdersDetail", otdc.OrdersTempDetail_GetList(Nothing, GridView1.GetFocusedRowCellValue("OS_BatchID").ToString))
        ...
        PreviewRPT(ds, "rptOrdersBom", " 物料清單報表", True, False)
        ltc = Nothing
        ltc1 = Nothing
        ltc2 = Nothing
        ltc3 = Nothing
    ...
    Private Sub popOrdersTempDeail_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles popOrdersTempDeail.Click
        '臨時單
        On Error Resume Next
        Dim ds As New DataSet
        If GridView1.RowCount = 0 Then Exit Sub
    ...
    Private Sub PopPJUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles PopPJUpdate.Click
        Dim Myfrm As New frmOrdersSubPJ
        Myfrm.ShowDialog()
    End Sub

    Private Sub popOrderSubSe_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles popOrderSubSe.Click
        Dim osc As New OrdersSubController
        Dim myfrm As New frmOutWardsFind          '@ 2012/9/12 改為調用frmOutWardsFind模塊
        myfrm.Text = "批次資料--查詢"
        myfrm.ShowDialog()
        If myfrm.isClickbtnOK = True Then
            Grid1.DataSource = osc.OrdersSub_GetList3(Nothing, tempValue2, tempValue6, "大貨批次", Nothing, Nothing, tempValue, tempValue3, tempValue5, tempValue4, tempValue8, tempValue9, tempValue7)
        End If
        tempValue = ""
        tempValue2 = ""
        tempValue3 = ""
    ...
    Private Sub popOrdersSum_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles popOrdersSum.Click
        'On Error Resume Next
        Dim ds As New DataSet
        If GridView1.RowCount = 0 Then Exit Sub
        ...
        Dim omc As New OrdersMainController
        ds.Tables.Clear()
        ltc2.CollToDataSet(ds, "OrdersSub", osc.OrdersSub_GetList(Nothing, Nothing, GridView1.GetFocusedRowCellValue("PM_M_Code").ToString, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing))

        PreviewRPT(ds, "rptOrdersSum", " 訂單數量匯總", True, False)
        ltc2 = Nothing

    End Sub

    Private Sub popOrdersSubNeed_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles popOrdersSubNeed.Click
        ...
        osi1 = osc1.OrdersSub_Get(GridView1.GetFocusedRowCellValue("OS_BatchID").ToString)
        If osi1.OS_Check = False Then
            MsgBox("此批次尚未審核,不能生成批次需求單！", 64, "提示")
            Exit Sub
        End If

        If onc.OrdersSubNeed_GetList(Nothing, Nothing, GridView1.GetFocusedRowCellValue("OS_BatchID").ToString, Nothing, Nothing, "W0301", Nothing, Nothing, Nothing, Nothing, Nothing, Nothing).Count > 0 Then
            MsgBox("此批次已生成需求單，不需要再生成！", 64, "提示")
            Exit Sub
        End If

        fr.txtOS_BatchID.Text = GridView1.GetFocusedRowCellValue(OS_BatchID)
        fr.txtOS_Qty.Text = GridView1.GetFocusedRowCellValue(OS_Qty)
        fr.lblTittle.Text = "批次需求單-新增"
        fr.ShowDialog()
    End Sub
End Class