...
Public Class frmPurchaseWLMain

    Private Sub frmPurchaseWLMain_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Select Case strTemp
            ...
            Case "采購員"
                Grid1.DataSource = mc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, "物料", Nothing, Nothing, Nothing, InUserID, "采購作業", Nothing, Nothing, False, tesuvalue)
            Case "采購主管"
                Grid1.DataSource = mc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, "物料", Nothing, Nothing, Nothing, Nothing, "采購作業", False, Nothing, False, tesuvalue)
            Case "會計部"
                Grid1.DataSource = mc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, "物料", Nothing, Nothing, Nothing, Nothing, "采購作業", True, False, False, tesuvalue)
            Case "管理員"
                Grid1.DataSource = mc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, "物料", Nothing, Nothing, Nothing, Nothing, "采購作業", Nothing, Nothing, False, tesuvalue)
        End Select

        LoadUserPower()
        Label1.Text = "采購管理--物料採購"
    ...
    Sub LoadUserPower()
        ...
        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020401")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then popPurchaseNew.Enabled = True
        End If


        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020402")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then popPurchaseEdit.Enabled = True
        End If

        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020403")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then popPurchaseDel.Enabled = True
        End If

        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020404")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then popPurchaseCheck.Enabled = True
        End If

        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020405")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then popPurchaseAccCheck.Enabled = True
        End If

        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020406")
        If pmwiL.Count > 0 Then
        ...
        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020408")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then popPurchasePrintEng.Enabled = True
        End If

        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020409")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then PurchaseRead.Enabled = True
        End If

        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020418")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then popPurchaseExport.Enabled = True : CopyALLToolStripMenuItem.Enabled = True
        End If


        pmwiL = pmws.PermissionModuleWarrantSub_GetList(InUserID, "40020421")
        If pmwiL.Count > 0 Then
            If pmwiL.Item(0).PMWS_Value = "是" Then popPurchaseFile.Enabled = False
        End If
    ...
    Private Sub popPurchaseDel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles popPurchaseDel.Click
        ...
        If (IsDBNull(pi.PM_AccountCheck) = True And IsDBNull(pi.PM_Check) = True) Or (pi.PM_AccountCheck = False And pi.PM_Check = False) Then
            ...
            If MsgBox("確定要刪除編號為" & GridView2.GetFocusedRowCellValue("PM_NO").ToString & "的所有採購單嗎？", MsgBoxStyle.YesNo, "刪除提示") = MsgBoxResult.Yes Then
                ...
                '@ 2012/6/14 添加 存在驗收記錄時，不允許刪除
                If ac.Acceptance_GetList(GridView2.GetFocusedRowCellValue("PM_NO").ToString, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing).Count > 0 Then
                    MsgBox("此採購單號 " & GridView2.GetFocusedRowCellValue("PM_NO").ToString & " 已存在驗收記錄，不允許刪除!", 64, "提示")
                    Exit Sub
                End If
                ...
                If pc.PurchaseMain_Delete(GridView2.GetFocusedRowCellValue("PM_NO").ToString) = True Then
                    ...
                    End If
                End If
                Grid1.DataSource = pc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, "物料", Nothing, Nothing, Nothing, Nothing, "采購作業", Nothing, Nothing, False, tesuvalue)
            End If
        Else
            MsgBox("此單已被審核，不允許刪除")
            Exit Sub
    ...
    Private Sub popPurchaseFile_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles popPurchaseFile.Click
        ...
        Dim pmws1 As New PermissionModuleWarrantSubController
        Dim pmwil1 As List(Of PermissionModuleWarrantSubInfo)
        Dim strTemp As String = ""
        pmwil1 = pmws1.PermissionModuleWarrantSub_GetList(InUserID, "40020417")
        strTemp = pmwil1.Item(0).PMWS_Value
        If strTemp = "非采購人員" Or strTemp = "會計部" Then

            FileShow("4002", GridView2.GetFocusedRowCellValue("PM_NO").ToString, True, False, False, False, False, False)
    ...
    Private Sub popPurchaseSeek_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles popPurchaseSeek.Click
        ...
        Select Case tempValue
            ...
            Case "1" '採購單號
                Grid1.DataSource = mc.PurchaseMain_Getlist(tempValue2, Nothing, Nothing, Nothing, Nothing, Nothing, "物料", Nothing, Nothing, Nothing, Nothing, "采購作業", Nothing, Nothing, Nothing, tesuvalue)
            Case "2" '物料規格
                Grid1.DataSource = mc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, Nothing, Nothing, tempValue2, "物料", Nothing, Nothing, Nothing, Nothing, "采購作業", Nothing, Nothing, Nothing, tesuvalue)
            Case "3" '物料名稱
                Grid1.DataSource = mc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, Nothing, tempValue2, Nothing, "物料", Nothing, Nothing, Nothing, Nothing, "采購作業", Nothing, Nothing, Nothing, tesuvalue)

            Case "4" '物料編碼 物料類型 
                Grid1.DataSource = mc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, tempValue2, Nothing, Nothing, "物料", Nothing, Nothing, Nothing, Nothing, "采購作業", Nothing, Nothing, Nothing, tesuvalue)

            Case "5" '供應商
                Grid1.DataSource = mc.PurchaseMain_Getlist(Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, "物料", Nothing, Nothing, tempValue2, Nothing, "采購作業", Nothing, Nothing, Nothing, tesuvalue)

            Case "6" '自定義查詢

                Grid1.DataSource = pc.PurchaseMain_Getlist("採購管理-物料", tempValue2)
    ...
    Private Sub popPurchaseExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles popPurchaseExport.Click
        ...
        'Dim fr As New FrmPurchaseExport
        'tempValue2 = "采購管理--物料採購"
        'fr.ShowDialog()
        ...
        'mc.PurchaseMain_Getlist(GridView2.GetFocusedRowCellValue("PM_NO").ToString, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
        'Grid1.DataSource = mc.PurchaseMain_Getlist1(Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, "物料", tempValue2, tempValue3, Nothing, Nothing, "采購作業", Nothing, Nothing, Nothing, tesuvalue)
        ...
        exbook.ActiveSheet.range(exsheet.Cells(1, 1), exsheet.Cells(GridView2.RowCount + 1, 24)).borders(1).linestyle = 1
        exbook.ActiveSheet.range(exsheet.Cells(1, 1), exsheet.Cells(GridView2.RowCount + 1, 24)).borders(2).linestyle = 1
        exbook.ActiveSheet.range(exsheet.Cells(1, 1), exsheet.Cells(GridView2.RowCount + 1, 24)).borders(3).linestyle = 1
        exbook.ActiveSheet.range(exsheet.Cells(1, 1), exsheet.Cells(GridView2.RowCount + 1, 24)).borders(4).linestyle = 1
    ...
    Private Sub 發送信息ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 發送信息ToolStripMenuItem.Click
